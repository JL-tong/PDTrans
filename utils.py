...
def load_checkpoint(checkpoint, model, autoencoder=None,optimizer=None):
    ...
    if autoencoder:
        autoencoder.load_state_dict(checkpoint['auto_dict'])
...
def update_metrics(raw_metrics, sample_mu, sigma, sample_90,labels, predict_start, sample=False, relative=False):   #  the original update!!!!! 
    ...
    raw_metrics['test_loss'] = raw_metrics['test_loss'] + [
        transformer.loss_fn(sample_mu.to('cpu'),sigma.to('cpu'), labels.to('cpu'),predict_start),1]
    if sample:
        raw_metrics['q90'] = raw_metrics['q90'] + transformer.accuracy_ROU(0.9, sample_90.to('cpu'), labels[:, predict_start:].to('cpu'), relative=relative)
        raw_metrics['q50'] = raw_metrics['q50'] + transformer.accuracy_ROU(0.5, sample_mu.to('cpu'), labels[:, predict_start:].to('cpu'), relative=relative)
    return raw_metrics
